...
test("states thats it is an Acute angle", function () {
   const currentOutput = getAngleType(38);
   const targetOutput = "Acute angle";
  expect(currentOutput).toBe(targetOutput);
});
test("states thats it is a Right angle", function () {
  const currentOutput = getAngleType(90);
  const targetOutput = "Right angle";
  expect(currentOutput).toBe(targetOutput);
});
test("states thats it is a Reflex angle", function () {
  const currentOutput = getAngleType(260);
  const targetOutput = "Reflex angle";
  expect(currentOutput).toBe(targetOutput);
});
test("states thats it is a Straight angle", function () {
  const currentOutput = getAngleType(180);
  const targetOutput = "Straight angle";
  expect(currentOutput).toBe(targetOutput);
});
test("states thats it is an Obtuse angle", function () {
  const currentOutput = getAngleType(143);
